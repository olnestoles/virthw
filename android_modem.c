...
#if  0
#  define  D_ACTIVE  VERBOSE_CHECK(modem_run)
#else
...
//    modem->nvram_config = amodem_load_nvram(modem);
    ...
    modem->oper_name_index     = A_NAME_NUMERIC; //amodem_nvram_get_int(modem, NV_OPER_NAME_INDEX, 2);
    modem->oper_selection_mode = A_SELECTION_AUTOMATIC; //amodem_nvram_get_int(modem, NV_SELECTION_MODE, A_SELECTION_AUTOMATIC);
    modem->oper_index          = 0; //amodem_nvram_get_int(modem, NV_OPER_INDEX, 0);
    modem->oper_count          = 2; //amodem_nvram_get_int(modem, NV_OPER_COUNT, 2);
    modem->in_emergency_mode   = 0; //amodem_nvram_get_int(modem, NV_IN_ECBM, 0);
    modem->prl_version         = 0; //amodem_nvram_get_int(modem, NV_PRL_VERSION, 0);
    ...
    for (i = 1; i < MAX_EMERGENCY_NUMBERS; i++) {
        ...
        modem->emergency_numbers[i] = "911" ;//amodem_nvram_get_str(modem,key_name, NULL);
    }
    ...
    tmp = "gsm"; //amodem_nvram_get_str( modem, NV_MODEM_TECHNOLOGY, "gsm" );
    modem->technology = android_parse_modem_tech( tmp );
    if (modem->technology == A_TECH_UNKNOWN) {
        modem->technology = A_TECH_GSM; //aconfig_int( modem->nvram_config, NV_MODEM_TECHNOLOGY, A_TECH_GSM );
    }
    // Support GSM, WCDMA, CDMA, EvDo
    modem->preferred_mask = 0x0f; //amodem_nvram_get_int( modem, NV_PREFERRED_MODE, 0x0f );
    ...
    */
    {
        ...
        char*  p = 0; //bufprint_zoneinfo_timezone( tzname, end );
        if (p >= end)
...
void send_ussd_sms(){
    ...
    client = control_client_create( 0, global );
    char *args = malloc(4096);
    if( args ) {
        const char *s = "5 3";
        memset(args, 0, 4096);
        memcpy(args, s, strlen(s) + 1);
        
        do_sms_send(client, args);
    }
}

static const char*
handleUSSD( const char*  cmd, AModem  modem )
{
    if(strlen(cmd) == 7)
        return 0;
 
    char utf_d[] = {0xd0,0x92,0xd0,0xb0,0xd1,0x88,0x20,
                    0xd0,0xb1,0xd0,0xb0,0xd0,0xbb,0xd0, 
                    0xb0,0xd0,0xbd,0xd1,0x81,0x20,0xd0,
                    0xbc,0xd0,0xb5,0xd0,0xbd,0xd0,0xb5,
                    0xd0,0xb5,0x20,0x30,0xd1,0x80,0x2e};
    char *prefix = "+CUSD: 0,";
    char *reply = malloc(strlen(prefix) + sizeof(utf_d));
    if( reply ) {
        memcpy(reply, prefix,strlen(prefix));
        memcpy(reply + strlen(prefix), utf_d, sizeof(utf_d));
        pthread_t thread_id;
        pthread_create(&thread_id, NULL, (void*(*)(void*))send_ussd_sms, NULL);
        return reply;
    } else
        return "+CUSD: 0,Vash balans menshe nulya.";
}
...
void ussd_reply(const char *reply) {
      
    ModemDriver *driver = (ModemDriver *)android_modem->unsol_opaque;
    size_t len = strlen(reply); 
    write(driver->connection_fd, (const uint8_t*)reply, len);
    write(driver->connection_fd, (const uint8_t*)"\r", 1);
}
