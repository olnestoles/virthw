...
modem_driver_unsol(void* _md, const char* message) {
    ...
    if (md->connection_fd > 0)
        write(md->connection_fd, (const uint8_t*) message, len);
}

/* despite its name, this function is called when the device writes to the modem */
void
modem_driver_read( void*  _md, const uint8_t*  src, int  len , int conn_fd)
{
    ModemDriver*      md  = _md;
    ...
    for ( ; src < end; src++ ) {
        ...
        if (c == '\n' || c == '\r') {
            ...
            if (answer != NULL) {
                ...
                printf("ASWER=%s\n", (const char*)answer);
                write(conn_fd, (const uint8_t*)answer, len);
                write(conn_fd, (const uint8_t*)"\r", 1);
            } else
                D( "%s: -- NO ANSWER\n", __FUNCTION__ );
...
static void
modem_driver_init( int  base_port, ModemDriver*  dm)
{
    dm->in_pos = 0;
    dm->in_sms = 0;
...
ModemDriver * android_modem_init(int base_port) {
    static ModemDriver modem_driver[1];
    modem_driver_init(base_port, modem_driver);
    android_modem = modem_driver->modem;
    return &modem_driver[0];
}
